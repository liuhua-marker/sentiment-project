  ...
  <div class="app-container box">
    ...
    <div class="app-table">
      ...
      <el-table v-loading="loading" :data="quanWordList" border>
        ...
        <el-table-column label="规则" prop="domainRule" />
        <el-table-column label="分类" prop="domainRuleClassify" width="200" />
        ...
        <el-table-column label="选择" width="200">
          <template slot-scope="scope">
            ...
            <el-button type="danger" size="small" @click="handleDelete([scope.row.domainRuleId])">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
    ...
    >
      ...
      >
        ...
        <el-form-item label="规则：" prop="domainRule">
          <el-input v-model.trim="ruleForm.domainRule" placeholder="请输入规则" />
        </el-form-item>
        <el-form-item label="规则分类：" prop="domainRuleClassify">
          <el-input v-model.trim="ruleForm.domainRuleClassify" placeholder="请输入分类信息" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getWordsTypeId() {
      ...
      await confwordstype(this.type).then((res) => {
        if (res.rows.length === 0) {
          this.$message({
            type: 'error',
            message: '请 先 到 规 则 类 型 管 理 配 置 领 域 规 则',
            showClose: true,
            duration: 5 * 1000
          })
        } else {
    ...
    ImportFiles() {
      ...
      this.action = confdomainrules() + this.queryForm.ruleTypeId
    },
    // 文件上传
    submitUpload() {
    ...
    clearDialog() {
      ...
      this.ruleForm.domainRule = ''
      this.ruleForm.domainRuleClassify = ''
