<template>
  <div class="app-container content_box">
    ...
    <el-table :data="resultList" style="width: 100%;margin-top:30px;" border>
      ...
      <el-table-column align="left" label="预警类别" width="80">
        <template slot-scope="scope">
          <el-popover
            placement="top-start"
            title
      ...
      <el-table-column align="left" label="摘要" width="150">
        <template slot-scope="scope">
          <el-popover
            ...
            trigger="hover"
            popper-class="popperClass"
            :content="scope.row.warnTypeText"
          >
...
<script>
import { findJZInternetByParams, findJZNoticeByParams } from '@/api/accurate'
...
export default {
  ...
  methods: {
    ...
    async findJZInternetByParams() {
      this.resultList = []
      const res = await findJZInternetByParams(this.queryForm)
    ...
    async findJZNoticeByParams() {
      this.resultList = []
      const res = await findJZNoticeByParams(this.queryForm)
      res.rows.forEach(row => {
        row.warnTypeText = this.viewWarnTypeText(row.warnType)
        this.resultList.push(row)
      })
